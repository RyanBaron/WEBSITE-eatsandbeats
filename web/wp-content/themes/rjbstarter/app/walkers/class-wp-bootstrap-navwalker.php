 ...
 if ( ! class_exists( 'WP_Bootstrap_Navwalker' ) ) :
   ...
   class WP_Bootstrap_Navwalker extends Walker_Nav_Menu {
     ...
     public function start_lvl( &$output, $depth = 0, $args = null ) {
      ...
      if ( $depth <= 0 ) {
         $classes = array( 'dropdown-menu', 'nav-lvl-0', 'animate', 'slideIn' );
      }
      elseif( $depth == 1 ) {
        $classes = array( 'section-menu', 'nav-lvl-1' );
      }
      elseif( $depth == 2 ) {
        $classes = array( 'sub-section-menu', 'nav-lvl-2' );
      }
      /**
     ...
     public function start_el( &$output, $item, $depth = 0, $args = null, $id = 0 ) {
        ...
        */
       $classes = self::separate_linkmods_and_icons_from_classes( $classes, $linkmod_classes, $icon_classes, $icon_pos, $depth );
      ...
      elseif ( $this->has_children && 1 === $depth ) {
         $atts['href']          = '#';
         $atts['id']            = 'menu-item-section-' . $item->ID;
       }
      else {
       ...
       if ( ! empty( $icon_class_string ) ) {
         ...
         $icon_html = '<span class="nav-icon-wrap"><i class="' . esc_attr( $icon_class_string ) . '" aria-hidden="true"></i></span>';
       }
        ...
        */
       //$title = apply_filters( 'nav_menu_item_title', $title, $item, $args, $depth );
      $desc = isset( $item->description ) && ! empty( $item->description ) ? '<div class="nav-desc">' . str_replace( '^', '"', str_replace(']', '>', str_replace( '[', '<', $item->description) ) ). '</div>' : '';
      $desc = isset( $item->description ) && ! empty( $item->description ) ? '<div class="nav-desc">' . str_replace(']', '>', str_replace( '[', '<', $item->description) ) . '</div>' : '';

      if( 'icon-after' === $icon_pos ) {
        // Put the item contents into $output.
        $item_output .= isset( $args->link_before ) ? $args->link_before . $title . $desc . $icon_html . $args->link_after : '';
      }
      else {
        $item_output .= isset( $args->link_before ) ? $args->link_before . $icon_html . $title . $desc . $args->link_after : '';
      }
      ...
      */
     private function separate_linkmods_and_icons_from_classes( $classes, &$linkmod_classes, &$icon_classes, &$icon_pos, $depth ) {
       ...
       foreach ( $classes as $key => $class ) {
         ...
         if ( preg_match( '/^disabled|^sr-only/i', $class ) ) {
           ...
           unset( $classes[ $key ] );
         } elseif ( preg_match( '/^dropdown-header|^dropdown-divider|^dropdown-section|^dropdown-item-text/i', $class ) && $depth > 0 ) {
           /*
           ...
           $linkmod_classes[] = $class;
           unset( $classes[ $key ] );
         } elseif ( preg_match( '/^nav-item-af-header-text/i', $class ) && $depth > 0 ) {
           $linkmod_classes[] = $class;
         } elseif ( preg_match( '/^fa-(\S*)?|^fa(s|r|l|b|c)?(\s?)?$/i', $class ) ) {
           // Font Awesome.
           $icon_classes[] = $class;
           unset( $classes[ $key ] );
           ...
           $icon_classes[] = $class;
           unset( $classes[ $key ] );
         } elseif ( preg_match( '/^icon-after/i', $class ) ) {
           // Glyphicons.
           $icon_pos = $class;
           //unset( $classes[ $key ] );
         }
       ...
       if ( ! empty( $linkmod_classes ) ) {
         foreach ( $linkmod_classes as $link_class ) {
           if ( ! empty( $link_class ) ) {
             ...
             } elseif ( 'nav-item-af-header-text' === $link_class ) {
               $linkmod_type = 'nav-item-af-header-text';
             } elseif ( 'dropdown-section' === $link_class ) {
               $linkmod_type = 'dropdown-section';
             }
           }
         }
       }
       ...
       if ( ! empty( $linkmod_classes ) ) {
         foreach ( $linkmod_classes as $link_class ) {
           if ( ! empty( $link_class ) ) {
              ...
              */
             if ( 'sr-only' !== $link_class && isset($atts['class'] ) ) {
             ...
             if ( 'disabled' === $link_class ) {
               ...
               $atts['href'] = '#';
               unset( $atts['target'] );
             } elseif ( 'dropdown-header' === $link_class || 'dropdown-divider' === $link_class || 'dropdown-item-text' === $link_class || 'dropdown-section' === $link_class || 'nav-item-af-header-text' === $link_class ) {
     ...
     private function linkmod_element_open( $linkmod_type, $attributes = '' ) {
       ...
       }elseif ( 'nav-item-af-header-text' === $linkmod_type ) {
         $output .= '<span class="nav-item-af-header-text-inside"' . $attributes . '>';
     ...
     private function linkmod_element_close( $linkmod_type ) {
       $output = '';
       if ( 'dropdown-header' === $linkmod_type || 'dropdown-item-text' === $linkmod_type || 'nav-item-af-header-text' === $linkmod_type ) {
         /*
          * For a header use a span with the .h6 class instead of a real
          * header tag so that it doesn't confuse screen readers.
          */
