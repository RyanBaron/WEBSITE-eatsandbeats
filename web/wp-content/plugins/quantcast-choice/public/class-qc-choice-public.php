...
use QCChoice\Values\QC_Choice_Values;  // This entire class and its functionality should be removed in the next TCF v2 update.
...
class QC_Choice_Public {
	...
	public function enqueue_scripts() {
		...
		if( empty( $this->qc_choice_cmp_utid ) || empty( $this->qc_choice_cmp_enable ) ) {
			...
			// TCF v1 Enabled (Scheduled deprecation of TCFv1 on August 16, 2020 by the IAB).
			wp_enqueue_script( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'js/script.min.js', array(), $this->version, false );
			wp_enqueue_script( 'qc-choice-init', plugin_dir_url( __FILE__ ) . 'js/script.async.min.js', array(), $this->version, false );
			...
			$choice_cmp_config = array(
				...
				'enabled' => $this->qc_choice_cmp_enable,
