<?php
/* No Cookie: means no opted out cookie, so no cookie actually means optied in. */
?>
...
<head>
  <meta charset="utf-8">
  <title>Rjbstarter Opt Out - No Cookie</title>
  <meta name="description" content="Rjbstarter Opt Out - No Cookie Iframe Page">
...
<body>
          ...
          <div class="message message--success">
            <div class="top-section">
              ...
              <div class="msg-wrapper">
                <p>You are not opted out of interest-based advertising from Rjbstarter.</p>
              </div>
            </div>
          </div>
          <div class="opt-strip__switch" data-behavior="opt_in_out">
            <span class="opt-strip__switch-label">Not Opted Out</span>
            <span class="opt-strip__switch-toggle"><span></span></span>
            <a id="opt-out-link" class="opt-strip__switch-label" href="#">Opt Out</a>
          </div>
        </div>
      </div>
    </div>
  ...
  <script>
    document.getElementById('opt-out-link').href = '//pixel.quantserve.com/optout?token=qc.com\u0026participant_id=1\u0026rd='+location.protocol+'//'+location.hostname+'\u0026action_id=4';
    $(document).ready(function(){
      ...
      $toggle.on('click', function(e){
        if(!$this.hasClass('active')){
          ...
          window.location = $label.prop('href');
          clicked = true;
        }
        e.preventDefault();
      });
